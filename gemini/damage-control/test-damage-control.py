...
Usage:
  # Interactive mode - test shell, edit, write hooks interactively
...
Examples:
  ...
  # Test bash hook blocks rm -rf (Gemini tool name)
  uv run test-damage-control.py bash run_shell_command "rm -rf /tmp" --expect-blocked

  # Test edit hook blocks zero-access path (Gemini tool name)
  uv run test-damage-control.py edit replace "~/.ssh/id_rsa" --expect-blocked

  # Test bash allows safe command
  uv run test-damage-control.py bash run_shell_command "ls -la" --expect-allowed
...
def prompt_tool_selection() -> Optional[str]:
    ...
    print("  [1] run_shell_command  - Test shell commands")
    print("  [2] replace            - Test file paths for edit operations")
    print("  [3] write_file         - Test file paths for write operations")
    print("  [q] Quit")
    print()
    ...
    while True:
        ...
        if choice in ('q', 'quit'):
            return None
        elif choice == '1' or choice in ('bash', 'run_shell_command'):
            return 'run_shell_command'
        elif choice == '2' or choice in ('edit', 'replace'):
            return 'replace'
        elif choice == '3' or choice in ('write', 'write_file'):
            return 'write_file'
        else:
...
# Map hook types and tool name aliases to hook scripts and canonical tool names
HOOK_MAP = {
    "bash":             ("bash-tool-damage-control.py", "run_shell_command"),
    "run_shell_command": ("bash-tool-damage-control.py", "run_shell_command"),
    "edit":             ("edit-tool-damage-control.py", "replace"),
    "replace":          ("edit-tool-damage-control.py", "replace"),
    "write":            ("write-tool-damage-control.py", "write_file"),
    "write_file":       ("write-tool-damage-control.py", "write_file"),
}


def get_hook_path(hook_type: str) -> Tuple[Path, str]:
    """Get path to hook script and canonical tool name."""
    entry = HOOK_MAP.get(hook_type.lower())
    if not entry:
        print(f"Error: Unknown hook type '{hook_type}'. Use: {list(HOOK_MAP.keys())}")
        sys.exit(1)
    script_name, tool_name = entry
    return get_script_dir() / script_name, tool_name
...
def run_test(hook_type: str, tool_name: str, value: str, expectation: str) -> bool:
    ...
    expectation can be: "blocked" or "allowed"
    Blocking is detected either by exit code 2 OR by {"decision": "deny"} in stdout.
    """
    hook_path, canonical_tool = get_hook_path(hook_type)
    # Allow caller to override the tool name (for flexibility), but default to canonical
    effective_tool = tool_name if tool_name else canonical_tool
    tool_input = build_tool_input(canonical_tool, value)
    ...
    # Detect block via exit code 2 OR JSON decision=deny
    blocked = exit_code == 2
    if not blocked and stdout:
        try:
