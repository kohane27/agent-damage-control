...
# ///
"""
Gemini CLI Replace Tool Damage Control
=======================================

Blocks edits to protected files via BeforeTool hook on replace tool.
...
def main() -> None:
    ...
    # Only check replace tool (Gemini's edit/replace tool)
    ...
    if blocked:
        ...
        print(
            json.dumps(
                {
                    "decision": "deny",
                    "reason": f"SECURITY: Blocked edit to {reason}: {file_path}",
                }
            )
        )
        sys.exit(0)
